...
class AnalyzeTools(BaseTool):
    ...
    async def build_planner_digest(self, session_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"[DIGEST] Building planner digest for session={session_id}")
        session = await self._session.get_session(session_id)
        findings = await self._session.get_findings(session_id)
        logger.info(f"[DIGEST] Session loaded: target={session.target if session else 'NONE'}, "
                     f"status={session.status if session else 'NONE'}, "
                     f"findings_in_db={len(findings)}")

        # Get already tested items from findings attack types
        ...
        logger.info(f"[DIGEST] Built: target={digest['target']}, "
                     f"already_tested={already_tested}, "
                     f"confirmed={len(confirmed)}, total={len(findings)}")
        return digest
