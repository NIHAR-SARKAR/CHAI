...
class ReconTools(BaseTool):
    ...
    async def run_passive(self, session_id: str, target: str, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"[RECON passive] START target={target} session={session_id}")
        findings = []

        # WHOIS lookup
        whois_cmd = f"whois {target}"
        logger.info(f"[RECON passive] Running WHOIS: {whois_cmd}")
        whois_result = await self._execute(whois_cmd, session_id, timeout=60)
        logger.info(f"[RECON passive] WHOIS result: rc={whois_result.get('returncode')}")
        ...
        logger.info(f"[RECON passive] Running DNS: {dns_cmd}")
        dns_result = await self._execute(dns_cmd, session_id, timeout=60)
        logger.info(f"[RECON passive] DNS result: rc={dns_result.get('returncode')}")
        ...
        logger.info(f"[RECON passive] END: {len(findings)} findings")
    ...
    async def run_active(self, session_id: str, target: str, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"[RECON active] START target={target} session={session_id}")
        findings: List[Finding] = []
        raw: Dict[str, Any] = {}

        # 1. Technology Fingerprinting
        logger.info(f"[RECON active] (1/5) Technology fingerprinting...")
        ...
        logger.info(f"[RECON active]   -> {len(tech_findings)} findings")

        # 2. API Endpoint Discovery
        logger.info(f"[RECON active] (2/5) API endpoint discovery...")
        api_findings, api_raw = await self._run_api_discovery(session_id, target)
        findings.extend(api_findings)
        raw["api_discovery"] = api_raw
        logger.info(f"[RECON active]   -> {len(api_findings)} findings")

        # 3. Swagger / OpenAPI Exposure
        logger.info(f"[RECON active] (3/5) Swagger/OpenAPI check...")
        ...
        logger.info(f"[RECON active]   -> {len(swagger_findings)} findings")

        # 4. Port & Service Scan
        logger.info(f"[RECON active] (4/5) Port scan...")
        port_findings, port_raw = await self._run_port_scan(session_id, target)
        findings.extend(port_findings)
        raw["port_scan"] = port_raw
        logger.info(f"[RECON active]   -> {len(port_findings)} findings")

        # 5. Directory & File Bruteforce
        logger.info(f"[RECON active] (5/5) Directory bruteforce...")
        dir_findings, dir_raw = await self._run_dir_bruteforce(session_id, target)
        findings.extend(dir_findings)
        raw["dir_bruteforce"] = dir_raw
        logger.info(f"[RECON active]   -> {len(dir_findings)} findings")

        logger.info(f"[RECON active] END: {len(findings)} total findings")
    ...
    async def _run_api_discovery(self, session_id: str, target: str) -> tuple:
        ...
        if not os.path.exists(api_wordlist):
            alt = os.path.join(os.sep, "usr", "share", "wordlists", "api-endpoints.txt")
            api_wordlist = alt if os.path.exists(alt) else api_wordlist
        ffuf_cmd = (
    ...
    async def _run_port_scan(self, session_id: str, target: str) -> tuple:
        ...
        nmap_cmd = f"nmap -sV -p 80,443,8060,8443,3000,5000,4000 {host}"
    ...
    async def _run_dir_bruteforce(self, session_id: str, target: str) -> tuple:
        ...
        if not os.path.exists(wordlist):
            alt = os.path.join(os.sep, "usr", "share", "wordlists", "common.txt")
            wordlist = alt if os.path.exists(alt) else wordlist
        tmp_fd, tmp_path = tempfile.mkstemp(suffix=".json", prefix=f"gobuster_{session_id}_")
        os.close(tmp_fd)
        gobuster_cmd = (
            f"gobuster dir -u {target} -w {wordlist} "
            f"-x php,html,js,json -t 30 -q -o {tmp_path} --format json"
        )
