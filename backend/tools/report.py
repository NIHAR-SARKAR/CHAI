...
def _get_reports_dir() -> Path:
    """Unified report directory under the CHAI temp root."""
    if sys.platform == "win32":
        base = Path(os.environ.get("LOCALAPPDATA", os.path.expanduser("~\\AppData\\Local"))) / "Temp" / ".chai" / "reports"
    else:
        base = Path("/tmp") / ".chai" / "reports"
