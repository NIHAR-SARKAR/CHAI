"""firejail/cgroups/chroot/wsl wrapper for safe command execution."""
import asyncio
import logging
import os
import signal
...
class ProcessController:
    def __init__(self, config, wsl_adapter=None, sudo_handler=None):
        self._config = config
    ...
    async def run(
        ...
        # ── WSL routing: on Windows with WSL active, route Linux commands into WSL ──
        if self._wsl and self._wsl.is_active() and sandbox_level != "wsl":
            # Check if command likely needs elevation (sudo)
            from core.sudo_handler import SudoHandler
            if SudoHandler.needs_sudo(command) and self._sudo:
                logger.info(f"[{session_id}] Command needs sudo — routing through WSL as root")
                return await self._sudo.run_elevated_wsl(
                    command=command,
            ...
            logger.info(f"[{session_id}] Routing command through WSL")
            return await self._wsl.run(
                command=command,
                session_id=session_id,
                timeout=timeout,
                working_dir=working_dir,
        ...
        try:
            ...
            try:
                ...
                # Detect sudo password prompt on native Linux and retry with elevation
                if sys.platform != "win32" and self._sudo:
                    from core.sudo_handler import SudoHandler
                    if SudoHandler.is_sudo_password_prompt(result.get("stderr", ""), result.get("stdout", "")):
                        logger.warning(f"[{session_id}] Native Linux: sudo password prompt detected — retrying with elevation")
                        return await self._handle_sudo_retry(command, session_id, timeout, result, working_dir)
                return result
    ...
    async def _handle_sudo_retry(
        ...
        """Detect sudo password prompt in result and retry with elevation if available."""
        if not self._sudo:
            return result
        from core.sudo_handler import SudoHandler
        if not SudoHandler.is_sudo_password_prompt(result.get("stderr", ""), result.get("stdout", "")):
            return result
        logger.warning(f"[{session_id}] Detected sudo password prompt — attempting elevation retry")
        ...
        return await self._sudo.run_with_password_linux(
            command=command,
            session_id=session_id,
            timeout=timeout,
            working_dir=working_dir,
    ...
    def _wrap_command(self, command: str, sandbox_level: str, max_ram_mb: int) -> str:
        ...
        if sandbox_level == "none":
            ...
            chroot_dir = getattr(self._config.paths, "chroot_dir", None) or "/var/chroot/pentest"
