...
class PluginLoader:
    def __init__(self, config, process_controller, safety_policy, session_manager, wsl_adapter=None, fallback_engine=None, sudo_handler=None):
        self._config = config
            ...
            try:
                ...
                for _, cls in inspect.getmembers(module, inspect.isclass):
                    if (issubclass(cls, PentestPlugin)
                        ...
                        # Check availability: on Windows with WSL active, also check inside WSL
                        available = await instance.is_available()
                        if not available and self._wsl and self._wsl.is_active():
                            available = await self._wsl.check_tool_available(instance.metadata.requires_binary)
